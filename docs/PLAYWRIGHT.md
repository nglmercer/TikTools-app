...
Browser-level end-to-end verification for the Vite/Vue SPA in Chromium.
...
devDependency and `playwright.config.ts` starts the app with
`bun run serve:web` automatically (`reuseExistingServer: true`).
...
- `state.actionDelayMs` delays action execution (speaking-state tests).
...
empty, connected creator, SonicBoom connected/disconnected, TTS/list/
form/connection pages, action/option failures, locales/themes) and
`tests/e2e/fixtures/plugins.ts` (SonicBoom-style descriptors, settings,
...
| `tts.spec.ts` | speak flow + speaking state + log, action failure, persistence coalescing |
| `connections.spec.ts` | empty state, probe success/failure, settings stay editable |
| `accessibility.spec.ts` | named nav + `aria-current`, labeled inputs, keyboard reachability, `status` errors |
| `screenshots.spec.ts` | semantic assertions + 14 deterministic baselines |
...
no remote avatars. The TTS log (timestamps) is masked in the error-state
shot; everything else renders fixed fixture data.
