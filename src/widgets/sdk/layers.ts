...
const oldDefaultHeadings: Record<string, Partial<Record<TextField, string>>> = {
  follow: { title: 'New follower' },
  share: { title: 'Shared' },
  gift: { title: 'Gift received', streakTitle: 'Gift streak' },
};

/** Drop headings generated by older defaults while preserving custom wording. */
export function removeDefaultHeadingLayers(kind: string, design: WidgetStyle): WidgetStyle {
  const headings = oldDefaultHeadings[kind];
  if (!headings) return design;
  const layers = design.layers?.filter((layer) => {
    const field = layer.field;
    return !(layer.kind === 'text' && field && layer.id === `field:${field}`
      && layer.text === headings[field] && layer.color === undefined
      && layer.fontSize === undefined && layer.fontWeight === undefined);
  });
  const text = design.text ? { ...design.text } : undefined;
  if (text) {
    for (const field of Object.keys(headings) as TextField[]) {
      if (text[field] === headings[field]) delete text[field];
    }
  }
  if ((!layers || layers.length === design.layers?.length)
    && (!text || Object.keys(text).length === Object.keys(design.text ?? {}).length)) return design;
  return { ...design, ...(layers ? { layers } : {}), ...(text ? { text } : {}) };
}
