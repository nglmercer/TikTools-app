/**
 * Subscribe Alert widget entry. Wires the shared runtime (credentials,
 * gateway client, reconnect) to the SubscribeController and renders alerts
 * with Vue.
 */
...
import { parseDemoMode, parseSubscribeSettings } from '../shared/config.ts';
...
const settings = parseSubscribeSettings(window.location.search);
const demo = parseDemoMode(window.location.hash);
const alert = ref<SubscribeAlert | null>(null);
...
if (demo === 'subscribe') {
  window.setTimeout(() => runtime.injectTestEnvelope(makeTestSubscribeEnvelope()), 600);
}
