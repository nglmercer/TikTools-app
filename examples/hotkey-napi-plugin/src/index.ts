...
type RdevKeyCode = string;

interface RdevInputEvent {
  eventType: string;
  keyPress?: { key: RdevKeyCode };
  keyRelease?: { key: RdevKeyCode };
  time: number;
}

interface RdevBinding {
  startListener(
    callback: (event: RdevInputEvent) => void,
    onError?: (message: string) => void,
  ): void;
  stopListener?: () => boolean;
}

const binding = require("rdev-node") as RdevBinding;
...
const pending: PressQueue = createPressQueue();
...
function pushPendingPresses(): void {
...
function statusEvent(droppedEvents: number, pendingEvents: number): PluginEvent {
...
const plugin: TikToolsPlugin = {
  ...
  async call(request, context) {
    ...
    if (request.type === "poll") {
      ...
      if (freshOverflow) {
        logs.push(
          `hotkey event queue overflowed: ${pending.dropped} event(s) dropped in total, ${pending.events.length} pending`,
        );
    ...
    if (request.type === "action") {
      ...
      if (typeId === "hotkey.bind" || typeId === "hotkey_bind" || typeId === "bind") {
        ...
        if (wantsListening(bindings, sequencesNeeded)) {
          ...
          logs.push(
            "staged rdev-node has no stopListener export; the native listener keeps running until unload",
          );
        }
        if (bindings.length > 0) {
          logs.push(
            "portal shortcuts unavailable in the napi-vm build (compositor chords unsupported); raw input covers all keys",
          );
        }
        ...
        return {
          summary: `hotkey bindings updated: ${bindings.length} chord(s) watched, sequences ${
            sequencesNeeded ? "enabled" : "disabled"
          }`,
          logs,
          intents: [],
          events: [],
      ...
      throw new Error(
        `unknown hotkey action ${JSON.stringify(typeId)}; use hotkey.bind, hotkey.status, or hotkey.diagnostics`,
      );
    }
