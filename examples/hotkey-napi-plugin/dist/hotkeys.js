...
export const STUCK_KEY_TIMEOUT_MS = 120000;
...
export function isModifier(name) {
...
export function canonicalModifiers(modifiers) {
...
export function emitPress(state, queue, name, pressed, backend, nowMs) {
...
export function drainBatch(queue) {
    return queue.events.splice(0, Math.min(queue.events.length, POLL_MAX_EVENTS_PER_RESPONSE));
}
export function chordEquals(left, right) {
    return left.key === right.key && left.modifiers === right.modifiers;
}
/// Stable shortcut id, e.g. `ctrl-shift-k`.
export function shortcutId(chord) {
    if (chord.modifiers === "") {
        return `key-${sanitize(chord.key)}`;
    }
    return `${chord.modifiers.split("+").join("-")}-${sanitize(chord.key)}`;
}
export function chordDescription(chord) {
    if (chord.modifiers === "") {
...
function normalizeKey(raw) {
    ...
    if ([...key].every((c) => /[A-Za-z0-9]/.test(c) || "-_=[]\\;',./` ".includes(c)) ||
        key.startsWith("f") ||
        NAMED.has(key)) {
        return key;
    }
...
export function parseBindConfig(config) {
    const shortcuts = typeof config === "object" &&
        config !== null &&
    ...
    shortcuts.forEach((entry, index) => {
        ...
        const modifiers = typeof record["modifiers"] === "string" ? record["modifiers"] : "";
        ...
        if (normalizedKey !== null && normalizedModifiers !== null) {
            ...
            warnings.push(`shortcut #${index} is not a chord (key=${JSON.stringify(key)}, modifiers=${JSON.stringify(modifiers)}); sequences stay on raw input`);
        }
    });
    const sequencesNeeded = typeof config === "object" &&
        config !== null &&
        typeof config["sequencesNeeded"] === "boolean"
        ? config["sequencesNeeded"]
        : true;
